<template>
  <div class="container">
    ...
    <ul class="nav nav-pills nav-stacked">
      <li v-for="item in leagues"  v-bind:key="item" role="presentation" class="active" v-on:click="fetchData(item.name)">{{item.title}}</li>
    </ul>
    <p></p>
    <div v-if="currentLeague">
      <h1>NYCL {{getLeagueTitle().title}}</h1>
      <div v-for="(division, index) in currentLeague.divisions"  v-bind:key="division" class="row col-sm">
        ...
        <table id="leagueTable" class="table table-bordered">
          ...
          <tr v-for="team in division.teams" v-bind:key="team">
            <td>{{team.clubName}}</td>
            <td>{{team.teamName}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      update: function () {
        axios.post('updateleagues', this.currentLeague)
      },
      download () {
